import { REQUIRED_TABLE, REQUIRED_FIELDS } from "./format";

export const validateDatabase = async (sql: any) => {
        try {
            // Check if 'image' table exists
            const tables = await sql`SELECT name FROM sqlite_master WHERE type='table' AND name=${REQUIRED_TABLE}`;
            ...
            if (missingFields.length > 0) {
                throw new Error(`Table '${REQUIRED_TABLE}' is missing required fields: ${missingFields.join(', ')}`);
            }
            ...
            for (const field of tableInfo) {
                if (REQUIRED_FIELDS.includes(field.name)) {
                    ...
                    if (field.type.toLowerCase() !== expectedType) {
                        console.warn(`Field '${field.name}' has type '${field.type}' but expected '${expectedType}'`);
                    }
                }
            }
