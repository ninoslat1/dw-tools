...
const NoHistory = () => {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            ...
            const tempFileHandle = await opfs.getFileHandle(file.name, {create: true})
            ...
            try {
                await validateDatabase(tempSql);
                 ...
                 try {
                    ...
                    alert(`Failed to clean up temp file: ${e instanceof Error ? e.message : "Internal Server Error"}`);
                }
            }
            ...
            const finalFileHandle = await opfs.getFileHandle(TARGET_DB_NAME, { create: true });
    ...
    return (
        ...
        >
            ...
            <p className='text-sm font-is mb-6 text-muted-foreground'>
                or drop the SQLite file <span onClick={handleClick} className="underline hover:cursor-pointer">here</span>
            </p>
